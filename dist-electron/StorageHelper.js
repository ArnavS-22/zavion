...
class StorageHelper {
    ...
    static tableName = 'activity_records';
    /**
    ...
    static initialize(url, key) {
        ...
        StorageHelper.supabase = (0, supabase_js_1.createClient)(url, key);
    }
    /**
     * Store a new activity record in Supabase.
     * @param record ActivityRecord object
     */
    static async storeActivityRecord(record) {
     ...
     */
    static getLocalDayBoundaries(date = new Date()) {
     ...
     */
    static async getDailyActivities(date = new Date()) {
        if (!StorageHelper.supabase) {
            throw new Error('Supabase client not initialized. Call StorageHelper.initialize first.');
        }
     ...
     */
    static async getHourlyBreakdown(date = new Date()) {
    ...
    static async getDailyStats(date = new Date()) {
        ...
        const focusCount = activities.filter(a => a.cognitive_state === 'deep_focus' || a.cognitive_state === 'light_work').length;
     ...
     */
    static async getActivitiesInRange(startDate, endDate) {
        if (!StorageHelper.supabase) {
            throw new Error('Supabase client not initialized.');
        }
