...
class LLMHelper {
    ...
    async retryWithBackoff(operation, maxRetries = CONFIG.MAX_PROMPT_RETRIES) {
        let lastError = null;
    ...
    validateActivityRecord(record) {
        ...
        return required.every(field => record.hasOwnProperty(field) && record[field] !== null && record[field] !== undefined);
    }
    ...
    async classifyProductivityActivity(imagePath) {
...
Analyze only what you can see. Return ONLY the JSON object.`;
    ...
    analyzeWorkSessions(activities) {
        const sessions = [];
        const gaps = [];
        const warnings = [];
        ...
        for (let i = 0; i < sortedActivities.length; i++) {
            ...
            if (!currentSession) {
                ...
                currentSession.durationMinutes = Math.round((currentSession.endTime.getTime() - currentSession.startTime.getTime()) / 1000 / 60);
    ...
    truncateActivitiesForPrompt(activities) {
    ...
    buildDailyInsightsPrompt(analysis, activities, isTruncated) {
...
${gaps.slice(0, 5).map(gap => `- ${gap.durationMinutes} min gap (${gap.reason}) at ${gap.startTime.toLocaleTimeString()}`).join('\n')}
...
- Acknowledge data quality issues if present`;
    ...
    async extractProblemFromImages(imagePaths) {
        try {
            const imageParts = await Promise.all(imagePaths.map(async (path) => ({
                inlineData: {
...
Important: Return ONLY the JSON object, without any markdown formatting or code blocks.`;
    ...
    async debugSolutionWithImages(problemInfo, currentCode, debugImagePaths) {
        try {
            const imageParts = await Promise.all(debugImagePaths.map(async (path) => ({
                inlineData: {
                    data: await this.readImageFile(path),
                    mimeType: "image/png"
                }
